"""
This file contains functions that map the content to html files.
"""
...
def raise_bid(request, auction_id):
    ...
    try:
        if request.session['username']:
            ...
            ## need to check if user is latest bidder, if not
            ## then only allow call to increase_bid()


            latest_bid = Bid.objects.filter(auction_id = auction_id).order_by('-bid_time')
            if len(latest_bid) !=0:
                if latest_bid[0].user_id.id == user.id:
                    pass
                else:
                    increase_bid(user, auction)
            else:
...
def my_items(request):
    """
    Records all the auctions that has been set up by a particular user
